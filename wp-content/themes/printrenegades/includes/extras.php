...
function printrenegades_mime_types($mimes){
    $mimes['svg'] = 'image/svg+xml';
    return $mimes;
}

add_filter('upload_mimes', 'printrenegades_mime_types');

add_filter('widget_text', 'do_shortcode');

add_filter('elementor/fonts/additional_fonts', 'printrenegades_elementor_custom_font');

function printrenegades_elementor_custom_font($additional_fonts)
{
    $additional_fonts = [
        'GothamProRegular' => 'system',
        'GothamProMedium' => 'system',
        'GothamProBold' => 'system',
        'GothamProBlack' => 'system',
        'GothamProItalic' => 'system',
        'FuturaRegular' => 'system',
        'FuturaBold' => 'system',
    ];
    return $additional_fonts;
}

add_shortcode('elementor_template_by_id', function ($atts) {
    $atts = shortcode_atts(array(
        'id' => '',
    ), $atts, 'elementor_template_by_id');
...
function breadcrumbs()
{
    ...
    $breadcrums_class = 'breadcrumbs-custom-path';
    $home_title = __('Home', 'printrenegades');
    ...
    if (!is_front_page()) {
        ...
        if (is_archive() && !is_tax() && !is_category() && !is_tag()) {
            ...
            // Paginated archives
            echo '<li class="item-current item-current-' . get_query_var('paged') . '"><span class="bread-current bread-current-' . get_query_var('paged') . '" title="Page ' . get_query_var('paged') . '">' . __('Page', 'printrenegades') . ' ' . get_query_var('paged') . '</span></li>';
            ...
            // 404 page
            echo '<li>' . __('Error 404', 'printrenegades') . '</li>';
        } elseif (is_home()) {
            // Blog
            echo '<li>' . __('Blog', 'printrenegades') . '</li>';
        }
...
//     locate_template('elementor-ext/class-elementor-create-element.php', true);
// }, 15);